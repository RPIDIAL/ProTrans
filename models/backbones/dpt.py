...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, embed_dim=768):
        ...
        # print("----self.H, self.W:",self.H, self.W)
...
class DeformablePatchTransformer(nn.Module):
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dims=[64, 128, 256, 512],
                 ...
                 depths=[3, 4, 6, 3], sr_ratios=[8, 4, 2, 1], patch_embeds=None, hidden_dim=2048, output_dim=768, dpt_return_stage=4):
        super().__init__()
        self.num_classes = num_classes
        self.depths = depths
        self.dpt_return_stage = dpt_return_stage
        # print("---img_size:",img_size)
        # print("---transformer-patch_size:",patch_size)
        # patch_embed
        ...
        self.local_embed = LocalEmbedding(
            embed_dims[self.dpt_return_stage -1], hidden_dim, output_dim
        )
        if self.dpt_return_stage != 4:
            self.norm_return = norm_layer(embed_dims[self.dpt_return_stage -1])
    ...
    def forward_features(self, x):
        ...
        if self.dpt_return_stage == 2:
            x = self.norm_return(x)
            return x
...
@register_model
def dpt_medium_256(pretrained=False, **kwargs):
    # patch_embed
    embed_dims=[64, 128, 320, 512]
    img_size = 256 #224
    Depatch = [False, True, True, True]
